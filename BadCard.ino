...
void processLine(String line)
{
  ...
  { // There is no space -> (1)
    if (
        line == "ENTER" ||
        line == "MENU" || line == "APP" | line == "DOWNARROW" || line == "DOWN" ||
  ...
  {                              // Delay before the next commande
  ...
  {                          // Command from (2)
    ...
    {                                     // For command in remaining commands
...
{
  saveEditFile = false;
  myFile = SD.open(root + "/" + fileName, FILE_WRITE);
  if (myFile)
  {
    for (int i = 0; i <= newFileLines; i++)
    {
  ...
  {
    display.println("Error occurred while saving file");
    myFile.close();
  }
}
...
void scriptMenu()
{
  char optionsList[3][20] = {"Execute script", "Delete script", "Edit script"};
