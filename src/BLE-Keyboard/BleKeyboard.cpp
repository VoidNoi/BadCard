...
// Method to set the keyboard layout
void BleKeyboard::setLayout(const uint8_t* layout) {
    _asciimapLayout = layout;
}
...
size_t BleKeyboard::press(uint8_t k)
{
	...
	if (k >= 136) {			// it's a non-printing key (not a modifier)
		...
		k = pgm_read_byte(_asciimapLayout + k);
		if (!k) {
			setWriteError();
			return 0;
		}
    if ((k & 0xc0) == 0xc0) {
            ...
            k &= 0x3F;
    } else if ((k & 0x80) == 0x80) {						// it's a capital letter or other character reached with shift
			_keyReport.modifiers |= 0x02;	// the left shift modifier
			k &= 0x7F;
		} 
	}

  if (k == 0x32) { // Is ISO_REPLACEMENT
            k = 0x64; // ISO_KEY
  }
