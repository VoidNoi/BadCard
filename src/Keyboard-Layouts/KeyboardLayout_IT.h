...
extern const char16_t KeyboardLayout_it_IT_special_characters[AMOUNT_OF_SPECIAL_CHARS] PROGMEM = {
  u'ì',
  u'è',
  u'ò',
  u'à',
...
extern const uint16_t KeyboardLayout_it_IT_special_keycodes[AMOUNT_OF_SPECIAL_CHARS] PROGMEM = {
  KEY_I_GRAVE,
  KEY_E_GRAVE,
  KEY_O_GRAVE,
  KEY_A_GRAVE,
  KEY_U_GRAVE,
  KEY_E_GRAVE | U16SHIFT,
  KEY_O_GRAVE | U16SHIFT,
  KEY_A_GRAVE | U16SHIFT,
...
class KeyboardLayout_IT : public KeyboardLayout
{
public:
  KeyboardLayout_IT()
  {
  ...
  uint16_t getKeycode(char16_t key)
  {
	...
	if (key < 128)
	  {
	    return KeyboardLayout_it_IT[key];
	  }
  ...
  uint16_t isSpecialKey(char16_t key)
  {
	...
	Serial.println((char16_t)KeyboardLayout_it_IT_special_characters[i]);
	
	if (key == KeyboardLayout_it_IT_special_characters[i])
	  {
	    Serial.println("Compare keys");
	    Serial.println(key);
	    Serial.println((char16_t)KeyboardLayout_it_IT_special_characters[i]);
	    Serial.println((char)key);
	    Serial.println((char)KeyboardLayout_it_IT_special_characters[i]);
	    
	    
	    keycode = KeyboardLayout_it_IT_special_keycodes[i];
