...
extern const uint16_t KeyboardLayout_pt_BR_special_keycodes[AMOUNT_OF_SPECIAL_CHARS] PROGMEM = {
...
extern const uint8_t KeyboardLayout_pt_BR[128] PROGMEM =
{
  ...
  0x32|SHIFT,    // |
  0x31|SHIFT,    // }
  0x21|ALT_GR,   // ~  not supported (requires dead key + space)
  0x00           // DEL
};

class KeyboardLayout_BR : public KeyboardLayout
{
public:
  KeyboardLayout_BR()
  {
  ...
  uint16_t getKeycode(char16_t key)
  {
	...
	if (key < 128)
	  {
	    return KeyboardLayout_pt_BR[key];
	  }
  ...
  uint16_t isSpecialKey(char16_t key)
  {
	...
	Serial.println((char16_t)KeyboardLayout_pt_BR_special_characters[i]);
	
	if (key == KeyboardLayout_pt_BR_special_characters[i])
	  {
	    Serial.println("Compare keys");
	    Serial.println(key);
	    Serial.println((char16_t)KeyboardLayout_pt_BR_special_characters[i]);
	    Serial.println((char)key);
	    Serial.println((char)KeyboardLayout_pt_BR_special_characters[i]);
	    
	    
	    keycode = KeyboardLayout_pt_BR_special_keycodes[i];
