...
extern const char16_t KeyboardLayout_fr_FR_special_characters[AMOUNT_OF_SPECIAL_CHARS] PROGMEM = {
  u'²',
  u'é',
  u'è',
  u'ç',
...
extern const uint16_t KeyboardLayout_fr_FR_special_keycodes[AMOUNT_OF_SPECIAL_CHARS] PROGMEM = {
  KEY_SUPERSCRIPT_TWO,
  KEY_E_ACUTE,
  KEY_E_GRAVE,
  KEY_C_CEDILLA,
...
public:
  ...
  uint16_t getKeycode(char16_t key)
  {
	...
	if (key < 128)
	  {
	    return KeyboardLayout_fr_FR[key];
	  }
  ...
  uint16_t isSpecialKey(char16_t key)
  {
	...
	Serial.println((char16_t)KeyboardLayout_fr_FR_special_characters[i]);
	
	if (key == KeyboardLayout_fr_FR_special_characters[i])
	  {
	    Serial.println("Compare keys");
	    Serial.println(key);
	    Serial.println((char16_t)KeyboardLayout_fr_FR_special_characters[i]);
	    Serial.println((char)key);
	    Serial.println((char)KeyboardLayout_fr_FR_special_characters[i]);
	    
	    
	    keycode = KeyboardLayout_fr_FR_special_keycodes[i];
