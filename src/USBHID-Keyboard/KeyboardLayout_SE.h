...
extern const char16_t KeyboardLayout_sv_SE_special_characters[AMOUNT_OF_SPECIAL_CHARS] PROGMEM = {
  u'å',
  u'ä',
  u'ö',
  u'¨',
...
extern const uint16_t KeyboardLayout_sv_SE_special_keycodes[AMOUNT_OF_SPECIAL_CHARS] PROGMEM = {
  KEY_A_RING,
  KEY_A_UMLAUT,
  KEY_O_UMLAUT,
  KEY_UMLAUT,
...
class KeyboardLayout_SE : public KeyboardLayout
{
public:
  KeyboardLayout_SE()
  {
  ...
  uint16_t getKeycode(char16_t key)
  {
	...
	if (key < 128)
	  {
	    return KeyboardLayout_sv_SE[key];
	  }
  ...
  uint16_t isSpecialKey(char16_t key)
  {
	...
	Serial.println((char16_t)KeyboardLayout_sv_SE_special_characters[i]);
	
	if (key == KeyboardLayout_sv_SE_special_characters[i])
	  {
	    Serial.println("Compare keys");
	    Serial.println(key);
	    Serial.println((char16_t)KeyboardLayout_sv_SE_special_characters[i]);
	    Serial.println((char)key);
	    Serial.println((char)KeyboardLayout_sv_SE_special_characters[i]);
	    
	    
	    keycode = KeyboardLayout_sv_SE_special_keycodes[i];
