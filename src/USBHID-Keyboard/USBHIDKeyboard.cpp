...
void USBHIDKeyboard::begin(KeyboardLayout *layout)
{
    if (layout != nullptr)
    {
...
void USBHIDKeyboard::setModifier(uint8_t modifier)
{
    _keyReport.modifiers |= modifier;
    sendReport(&_keyReport);
}

void USBHIDKeyboard::unsetModifier(uint8_t modifier)
{
    _keyReport.modifiers &= ~(modifier);
    sendReport(&_keyReport);
}
...
size_t USBHIDKeyboard::press(uint16_t key)
{
    if (key > 0x88)
    {
...
size_t USBHIDKeyboard::release(uint16_t key)
{
    if (key > 0x88)
    {
        uint16_t keycode = keyboardlayout->isSpecialKey(key);
...
size_t USBHIDKeyboard::write(uint16_t k)
{
    uint8_t p = press(k); // Keydown
    release(k);           // Keyup
    return p;             // just return the result of press() since release() almost always returns 1
}
size_t USBHIDKeyboard::write(const uint16_t *buffer, size_t size)
{
    size_t n = 0;
    while (size--)
    {
...
size_t USBHIDKeyboard::write(std::u16string text)
{
